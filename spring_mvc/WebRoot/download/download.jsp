...
<%
	...
	String genreID = "0";
	if (!request.getParameter("genreID").equals("null") &&request.getParameter("genreID") != null) {//!request.getParameter("genreID").equals("null") && 
...
<script type="text/javascript">
  ...
  <div class="browser-questions">
			<ul class="breadcrumb" style="margin:80px 0 10px 30px;">
				<li><a href="index.jsp?linknumber=1">主页</a>
				</li>
  	...
  	for( Dowload down : pageInfo.getDownloads()){
  		boxQuestionNum++;
  ImageIcon imgiconPhoto1 = null;  
  int height = 0;  
  int width = 0;  
  String imagepath = request.getSession().getServletContext().getRealPath("user/userimg/"+down.getAnswerImage());
  //out.print(imagepath);
  try{
     ...
     // out.print(width + "  ,  " + height);
  	...
  	<div class="down">
    	<div class="down_hear">
    		...
    		<div class="hearimg"><img alt="图片加载中......" src="user/userimg/<%=down.getAnswerImage() %>"
    	...
    	<div class="question">
    		<a style="text-decoration: none;" href="ScanDownQuestion?QuestionID=<%=down.getQuestion().getQuestionId()%>&linknumber=1"><h4><%=down.getQuestion().getQuestionTheme() %></h4></a>
    		<p>内容：<%=down.getQuestion().getQuestionContent().replace("\n", "</p><p>") %></p>
    	</div>
    	
    	<div class="down-right">
    		<div style="margin-top: 5px;float: left;"><h6>下载花费：</h6>
    		<span style="font-size: 30px;padding-left : 20px;"><%=down.getQuestion().getQuestionDownMoney() %></span>
    	...
    	<div style="clear: left;" class="wdc_p">
    		<a id="a<%=down.getQuestion().getQuestionId()%>" style="text-decoration: none; cursor: pointer;"><i>解答者：<span><%=down.getUser().getUserName() %></span></i> </a> 
    		<a id="b<%=down.getQuestion().getQuestionId()%>" style="text-decoration: none;" href="ScanDownQuestion?QuestionID=<%=down.getQuestion().getQuestionId()%>"><i>评论个数<span><%=down.getQuestionCommentNum() %></span></i> </a>  
    		<a id="c<%=down.getQuestion().getQuestionId()%>" style="text-decoration: none; cursor: pointer;" onclick="zanfun(<%=down.getQuestion().getQuestionId()%>)"><i>给TA一赞<span id="dz<%=down.getQuestionID().replace("0", "")%>"><%=down.getQuestionLikeNum() %></span></i> </a >
    		<a id="d<%=down.getQuestion().getQuestionId()%>" style="text-decoration: none; cursor: pointer;" ><i>下载次数<span id="xz"><%=down.getQuestionDownNum() %></span></i> </a >
    	</div>
    	<!-- 显示评论
