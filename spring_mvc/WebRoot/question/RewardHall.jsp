					...
					<div class="bd">
						<ul>
							<li
								...
								on <%}%>"><a
								href="RewardHall?linknumber=1">全部</a></li>
							<li
								class="course-nav-item
                                <%if (genreType == 1) {%>
								on <%}%>">
								<a href="RewardHall?genreType=1&linknumber=1"
							...
							<li
                               ...
                               <%if (genreType == 2) {%>
								on <%}%>">
								<a href="RewardHall?genreType=2&linknumber=1"
							...
							<li
                                ...
                                <%if (genreType == 3) {%>
								on <%}%>">
								<a href="RewardHall?genreType=3&linknumber=1"
							...
							<li
                                ...
                                <%if (genreType == 4) {%>
								on <%}%>">
								<a href="RewardHall?genreType=4&linknumber=1"
					...
					<div class="bd">
						<ul class="">
							<li
								...
								on <%}%>"><a
								href="RewardHall?genreType=<%=genreType%>&linknumber=1">全部</a>
							</li>
							<%
							...
							<li
								...
								on <%}%>"><a
								href="RewardHall?genreID=<%=g.getGenreId()%>&genreType=<%=genreType%>&linknumber=1&"
					...
					<div class="bd">
						<ul class="">
							...
							<li class="course-nav-item "><a
								href="RewardHall?genreType=<%=genreType%>&genreID=<%=genreID%>&linknumber=1&<%if (sortRule.equals("1")) {%>sortRule=2<%} else {%>sortRule=1<%}%>">时间<span
							...
							<li class="course-nav-item"><a
								href="RewardHall?genreType=<%=genreType%>&genreID=<%=genreID%>&linknumber=1&<%if (sortRule.equals("3")) {%>sortRule=4<%} else {%>sortRule=3<%}%>">浏览量<span
							...
							<li class="course-nav-item"><a
								href="RewardHall?genreType=<%=genreType%>&genreID=<%=genreID%>&linknumber=1&<%if (sortRule.equals("5")) {%>sortRule=6<%} else {%>sortRule=5<%}%>">悬赏金额<span
				...
				<div class="box-question">
					<div style="height: 50px;margin: 3px;font-size: 14px;">
						<img src="img/help.png"><a
							href="ScanQuestion?QuestionID=<%=q.getQuestionId()%>&linknumber=1"><%=q.getQuestionTheme()%></a>
					</div>
					<hr style="margin: 0px;">
					...
					%>
					<div
						style="float:right; width: 75px; margin-top: -20px; background-color: #F0F0F0">
					...
					<p>
						悬赏者：<%=q.getUser().getUserName()%></p>
					<p>
						分类：<b><%=q.getQuestionGenre()%></b>
					</p>
					<p>
			...
			<div style="clear:both;margin-left: 50px;">
				...
				<ul class="pagination">
					<li <%if (pageInfo.getPageNum() == 1) {%> class="disabled"
						title="已是第一页" <%}%>><a
						href="RewardHall?pageNum=<%=pageInfo.getPageNum() == 1 ? 1 : pageInfo
					...
					<li <%if (i + 1 == pageInfo.getPageNum()) {%> class="active" <%}%>><a
						href="RewardHall?linknumber=1&pageNum=<%=i + 1%>&ScanType=myScan&genreType=<%=genreType%>&sortRule=<%=sortRule%>&genreID=<%=genreID%>"><%=i + 1%></a>
						<%
							}
						%>
					...
					<li <%if (pageInfo.getPageSum() == pageInfo.getPageNum()) {%>
						...
						href="RewardHall?linknumber=1&pageNum=<%=(pageInfo.getPageNum() + 1) > pageInfo.getPageSum() ? pageInfo
